...
<!--
  ...
  docs/framework_exceptions/dev_framework_exceptions.md. See FWADR-014.
-->
...
|---|---|---|
| "you are the Strategist" | Strategist | [`docs/dev_framework/strategist.md`](docs/dev_framework/strategist.md) + [`dev_framework_exceptions.md`](docs/framework_exceptions/dev_framework_exceptions.md) + planning docs (plan, roadmap, future-directions). Does NOT load project `src/`. Also owns **operational docs** (runbooks, SOPs, checklists). Writes the artifact you named at the path you named — substituting an adjacent one needs your OK first, and a missing ADR is a question for you, not tickets to create ([FWADR-025](docs/dev_framework/adrs/fwadr-025-named-deliverables.md)). |
...
| "you are the Researcher" | Researcher | [`docs/dev_framework/researcher.md`](docs/dev_framework/researcher.md) + [`dev_framework_exceptions.md`](docs/framework_exceptions/dev_framework_exceptions.md) (incl. the project's Researcher parameters entry) + the acquisition reading list that entry names. **Optional, per-project** — refuses to bootstrap unless `RESEARCHER_SCOPE_DIR` (`.env`) and the parameters entry exist. Finder, not builder: writes fenced to one directory (mechanical check), merges to `dev` only, files `req-*` server-work requests instead of touching server code ([FWADR-024](docs/dev_framework/adrs/fwadr-024-researcher-role.md)). |
| "you are the Curator" | Curator | [`docs/dev_framework/curator.md`](docs/dev_framework/curator.md) + [`dev_framework_exceptions.md`](docs/framework_exceptions/dev_framework_exceptions.md), then runs [`./scripts/doc_churn_audit.sh`](scripts/doc_churn_audit.sh). Audits the **accumulated** doc corpus — over-churned ADRs, dead ADRs, closed plans to archive, exceptions past their retire-when. Holds the anti-accumulation incentive the Strategist structurally cannot. **Proposes, never disposes** — per-item user confirmation before anything is archived or deleted. Fenced out of `docs/dev_framework/*` (mechanical check); framework findings become `fwreq-*` requests you carry to the Template Developer ([FWADR-026](docs/dev_framework/adrs/fwadr-026-curator-role.md)). |
...
**Pushing `main` is mechanically blocked** by a git pre-push guard the framework sync installs into every code repo ([FWADR-023](docs/dev_framework/adrs/fwadr-023-main-push-guard.md)). The only sanctioned push path is `./scripts/promote_dev_to_main.sh` from `$PROJECT_DIR` (phase-exit promotion; `--bypass` for the emergency path). Never push `main` any other way; never set `FRAMEWORK_ALLOW_MAIN_PUSH` by hand or in `.env`.
...
**Canonical layout: split** ([FWADR-021](docs/dev_framework/adrs/fwadr-021-split-layout.md)). Claude Code is invoked from a parent directory (`$PROJECT_DIR`) that holds tracking material (`CLAUDE.md`, `docs/`, `.claude/`, `.mcp.json`). The git repo lives at `$PROJECT_DIR/$CODE_SUBDIR`. `$CODE_ROOT = $PROJECT_DIR/$CODE_SUBDIR`.

Set `DEFAULT_CODE_SUBDIR=<repo-slug>` in `$PROJECT_DIR/.env`. For multi-repo projects, W-item files set `target-repo: <subdir>` in their YAML frontmatter (per [FWADR-020](docs/dev_framework/adrs/fwadr-020-yaml-frontmatter-w-items.md)) to override the default.

**`$PROJECT_DIR` git tracking is optional.** Two sanctioned modes ([FWADR-021](docs/dev_framework/adrs/fwadr-021-split-layout.md) §"`$PROJECT_DIR` git tracking: optional"): *untracked parent* (default, simpler — plan-write visibility via shared filesystem only) or *tracked parent* (optional — `$PROJECT_DIR` is its own git repo for full PLAN-WRITE DISCIPLINE concurrent-claim safety + durable plan history). Code-side `git push origin dev/main` always operates on `$CODE_ROOT`, independent of which parent mode.
...
1. `.claude/hooks/sync-framework.sh` — destructively syncs `docs/dev_framework/` and `.claude/hooks/` from the canonical `claude_template_yaml` repo, initializes `docs/framework_exceptions/` if missing, and refreshes this managed block. Adopters are expected to make changes ONLY in `docs/framework_exceptions/*`, never in `docs/dev_framework/*`. See [FWADR-014](docs/dev_framework/adrs/fwadr-014-framework-sync-on-session-start.md).
2. `.claude/hooks/session-reorient.sh` — injects a role re-orientation reminder tailored to the `source` of the reset. See [FWADR-012](docs/dev_framework/adrs/fwadr-012-auto-reorient-hook.md).
...
# Dev slots (FWADR-019) — one-time setup, then launch/teardown by slot name.
# Run these from $PROJECT_DIR (parent), not from $CODE_ROOT — scripts live at
# the parent under split layout per FWADR-021 §"Script placement doctrine" and use
...
./scripts/promote_dev_to_main.sh         # THE ONLY sanctioned way to push main (FWADR-023);
