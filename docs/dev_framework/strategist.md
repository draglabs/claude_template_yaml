...
- **Writes operational docs (runbooks, procedures, checklists).** A first-class Strategist artifact class, distinct from planning docs: a *planning* doc says what will be built and why; an *operational* doc says how a human or agent performs a recurring procedure that already exists — orient, run, verify, recover. Runbooks, SOPs, and operator checklists are all this class. **Location is project-chosen** — the framework prescribes no `docs/runbooks/` convention; if the user names a path or an existing sibling document ("next to the other runbook"), that path is authoritative and is not a decision to re-open. A runbook that describes an agent operating a capability is a *description of an existing procedure*, not a proposal to expand scope — do not treat it as unplanned work, and do not answer a request for one by writing a constraint against the capability it documents.
...
- **Triages server-work requests (`req-*.md`) filed by the Researcher.** In projects with an active [Researcher](researcher.md) role (FWADR-024), scan for `req-*.md` files **at every Strategist session start and at phase boundaries** — in the active plan folder(s) under `docs/execution-plans/` and in `docs/execution-plans/inbox/`. Each `status: open` request gets a disposition recorded on its frontmatter: **accepted** (fold into a plan as a W-item; record the W-id, move an inbox file into that plan's folder), **declined** (one-line reason), or **deferred** (name the re-entry criterion). Requests state a desired outcome, not a design — the Strategist owns the how/whether. User involvement follows the same rule as claims: accepting a request is a scope decision, so present it to the user before folding it into a plan. If the project activates the Researcher, the Strategist also owns writing its **Researcher parameters** entry in `dev_framework_exceptions.md` and the `RESEARCHER_*` values in `.env` (see `researcher.md` §"Parameter surface").
...
## Named deliverables (never substitute silently)

The Strategist's write surface is enumerated above, and its most practised verbs are *triage, dispose, flip Status*. That combination has a specific failure mode: a request for something the enumeration doesn't obviously cover gets answered by writing the nearest thing the role **is** permitted to write — usually a W-item, a plan edit, or a disposition — instead of the thing that was asked for. The substitution happens at high confidence, so no uncertainty-based check catches it. This section is the check.

**The trigger is objective, not self-rated.** Do not route this through the confidence ladder — that ladder fires on an honest "I'm not sure which way," and silent substitution feels certain. Ask instead:

> Did the request name an artifact, a document type, or a location?

If yes, **that named artifact is the deliverable.** Not an approximation of it, not an adjacent ledger item, not a note recorded onto something that already exists. Producing anything else *instead* is a substitution, and substitution requires the user's agreement **before** it happens, never a report of it afterward.

**Two firing moments, both user-visible.**

1. **Before the first write** — state the comparison in one line: *"You asked for `<artifact>` at `<path>`; writing `<artifact>` at `<path>`."* State it as a comparison, not as an intention. "I'll honor the request" is satisfied by anything; "you asked for X at P, I am writing X at P" is falsifiable by the user reading it.
2. **At completion** — state what was produced against what was named. If they differ in type, name, or location, say so explicitly and say why. A mismatch is then an objective comparison the user can catch, not a judgment call buried in prose.

**Requests outside the write surface.** If the named artifact genuinely falls outside what this role may write, the response is to **say so and ask** — name the artifact, name the boundary that blocks it, and propose the role or path that can produce it. Never resolve an out-of-surface request by writing an in-surface artifact and calling it done. An unmapped request is a question for the user, not a routing puzzle to solve alone. Note that operational docs (above) **are** in surface — a runbook is not an out-of-surface request.

### Missing definitions are questions, not prerequisites

The most expensive form of this failure is not substituting the artifact — it is **manufacturing the artifacts the deliverable seemed to depend on.** While writing a requested doc, the Strategist needs a value: a threshold, a cadence, a model tier, a retention window. It looks for the ADR or plan entry that defines it. There isn't one. It then treats the missing ADR as a *blocking prerequisite*, and files work items to create the ADRs that would define the parameters — and the originally requested document never gets written. That is a rabbit hole, and it is the framework's fault, not the model's: the Strategist has a deeply-worn path from "gap found" → "work item created" (§"Triages `process-exceptions.md`," §"Triages server-work requests" — both dispose gaps by folding them into a plan) and **no path at all** from "gap found" → "ask the user for the value."

**The rule: when authoring a requested document requires a value nothing defines, ask the user for the value.** One question, answered in one line, in the same conversation. Do not create an ADR, a W-item, a plan amendment, or a `req-*` to define it first. The user is present; they are the source of that value; asking costs one exchange and manufacturing costs a phase.

**"Docs before code" does not apply here.** That rule (CLAUDE.md §"Two process rules") gates *implementing* an architectural addition — code must not land ahead of the doc describing it. It says nothing about *authoring* a document, and it does not mean every value appearing in a doc must first exist in an ADR. An operational doc may record a parameter the user states on the spot; that is the document doing its job, not an undocumented decision sneaking in. If a value later proves genuinely architectural, note it inline (`<!-- parameter set by user YYYY-MM-DD; promote to ADR if it becomes load-bearing -->`) and move on. Capturing a value is not the same as locking a decision.

**Escalating a prerequisite is a user decision, not a Strategist one.** If the missing definition is genuinely too large to settle in one question — it needs real architectural work, not a value — then **say that and stop**: name the deliverable, name the specific parameter blocking it, name what settling it would cost, and ask whether to (a) proceed on a stated assumption and mark it, (b) settle it now in conversation, or (c) pause the deliverable and do the upstream work first. Option (c) is the one that requires explicit user agreement, because it converts a one-document request into a phase of work. Choosing (c) unilaterally is the failure this section exists to prevent.

**Related but not the same:** this section handles *certainty pointed at the wrong object*. For genuine uncertainty within an accepted deliverable, use the ladder in §"Escalation and proportionality" below. Both can apply to the same task.

## Escalation and proportionality

Two rules for work the Strategist has correctly accepted. They address different halves of the same complaint — "it asked me nothing, and it did far too much."

**The 80/20 confidence ladder.** Same ladder the Developer runs — see [`developer.md`](developer.md) §"Confidence-driven escalation" for the full statement, the 80% threshold's rationale, and the false-high / false-low bias notes. In brief: self ≥80% on one option → act, don't ask; self <80% → spawn a consultant subagent with the fork packaged into its brief; consultant round-trip still <80% → escalate to the user, naming the fork rather than asking "what do you think?"

**Strategist calibration on rung 2.** The consultant rung costs a round-trip and returns context into this session's window. Key it on availability and scope: when **the user is in the loop and the task is bounded** — a one-off intake, a single doc edit, a question asked in conversation — skip rung 2 and go straight to asking them. A consultant round-trip costs more context than the question is worth when the person who knows the answer is already present. Rung 2 stands when the user is not available, or when the gap is a fact about the code or doc corpus rather than a decision only the user can make (that is the Code Consultant's primary use — see §"Staying code-aware without loading code").

**Proportionality — match the response to the request.** The ladder governs decision forks; it does not govern how much work a task becomes. This rule does. **A bounded task gets a bounded response.** When handed a specific, small job — intake this ticket, update this doc, record this decision — do that job and stop. Do NOT, unasked, additionally run an alignment audit, a stub audit, a cross-reference sweep, a status-ledger reconciliation, or a coherence pass across the corpus. Those are phase-boundary responsibilities (§"Stub audit" names its own cadence: phase boundaries, on demand, after a framework version jump) — they are not free additions to every task that touches a doc.

The failure this prevents: the role doc above is written as a *standing job description* for a persistent session. Read as a task list at the moment of a small request, it inflates a five-minute intake into a corpus-wide review that exhausts the context window and never returns the small thing that was asked for. If, mid-task, the work has grown well past what was requested, that is the signal — stop, state what the request was, state what the work has become, and ask whether to continue. Noticing scope inflation is the Strategist's job; absorbing it silently is not.

**Mechanism.** The forcing function is that both statements are **output the user sees and can interrupt** — the same shape the framework already sanctions where an agent cannot verify something generically: [`developer.md`](developer.md) §"Lifecycle (per W-item)" ("Before any code, the Developer asks the user 'Ready to start coding W-X?'") and [`dev-environment.md`](dev-environment.md) §"Pointing prod at a non-main branch (escape hatch)" ("Both ask the user directly because the CI surface is project-specific — the agent can't probe it generically"). No script ships with this rule; it is enforced at the two moments above and re-anchored after context resets by `.claude/hooks/session-reorient.sh`. See [FWADR-025](adrs/fwadr-025-named-deliverables.md).
...
The Strategist's very first session on a fresh project (after the template has been initialized into a parent directory) runs a structured intake with the user. The goal: a confirmed `claude-env` launcher in the user's shell rc (Block 0), a filled `$PROJECT_DIR/.env`, a CLAUDE.md with no surviving `{{...}}` placeholders, a slots.yaml with the right shape, and a working `.mcp.json` (every server connected or explicitly waived) — BEFORE any other role does meaningful work or before the Developer runs `scripts/setup_dev_slots.sh`.

This isn't optional — `scripts/setup_dev_slots.sh` sources `.env` and halts on `PLACEHOLDER` values for `PROJECT_SUB` or `PROJECT_PORTS` (see [FWADR-019](adrs/fwadr-019-dev-slots-and-deploy-stubs.md) Revision v1.1), so the Developer's first launch attempt will fail until the Strategist has done this.
...
**Block 0 — launch environment (fills the user's shell rc; [FWADR-032](adrs/fwadr-032-claude-env-launcher.md)):**

Everything downstream assumes Claude Code was launched with `.env` in its process env — MCP credentials and the FWADR-030 git-auth auto-wire both silently fail otherwise. The canonical launch path is the `claude-env` launcher pair (snippet: [`_stubs/shell/claude-env.zsh`](_stubs/shell/claude-env.zsh)): `claude-env` (safe, normal permission prompts) and `claude-env-yolo` (adds `--dangerously-skip-permissions`). The choice between them is made at every launch by name — nothing stores a permission posture.

| Question | Where it lands |
|---|---|
| Is the `claude-env` launcher installed in your shell rc? (With the user's go-ahead — the rc is outside the project tree — check: `grep -q 'claude-env' ~/.zshrc`.) If not: present both variants and ask which to install — safe only, or safe + yolo. Offer either a `source <path-to-snippet>` line or pasting the functions verbatim. | The user's shell rc (e.g. `~/.zshrc`), edited only on explicit approval. Never installed silently. |
| Confirm the user knows the consequence of skipping: every session launched bare gets the sync hook's step-0 NOTICE (unbound MCP creds + git auth) until relaunched via `claude-env`. | Nowhere — this is the informed-decline path. The NOTICE is the standing enforcement. |
...
| Recommend enabling **host-side branch protection on `main`** (belt-and-braces over the FWADR-023 pre-push guard, which is machine-local): GitHub → repo Settings → Branches; GitLab → protected branches. For `GIT_HOST=other`, record whether an equivalent exists in `dev_framework_exceptions.md`. | (no .env mirror — configured on the git host) | (none) |
...
| `docs/dev/slots.yaml` with `port: 0` | Unconfigured dev slots ([FWADR-019](adrs/fwadr-019-dev-slots-and-deploy-stubs.md)) | RUN `./scripts/setup_dev_slots.sh` once. Not a "remove" — a "fill it in." |
| `scripts/launch_local.sh` / `teardown_local.sh` / `main_to_prod.sh` with stub `exit 1` body | Unfilled deploy stubs ([FWADR-019](adrs/fwadr-019-dev-slots-and-deploy-stubs.md)) | FILL the body (most projects: never need to fill `main_to_prod.sh` — `exit 1` IS the correct steady state for CI-deployed projects). Not a "remove." |
