...
**When the user's instruction conflicts with an accepted ADR, the ADR is what changes.** Not the instruction, and not the code. The ADR log is living architecture — the best current record of decisions — not doctrine that outranks the person the architecture is for. See [ADR-027](../architecture/adr-027-living-architecture.md).
