...
- **Owns `docs/dev_framework/adrs/fwadr-*` for framework decisions.** ADRs that document *framework* choices (hooks, peer-dispatch, sync mechanism, role splits) live in their own `fwadr-NNN` number space inside the synced tree, so every adopter receives them and their links resolve everywhere ([FWADR-028](adrs/fwadr-028-framework-adr-namespace.md)). `docs/architecture/adr-NNN` is the *product* ADR space — it belongs to a product project's Strategist, and the framework never writes into it (the seed stubs `adr-000`/`stack.md`/etc. excepted).
...
- **Curates the framework corpus itself.** The [Curator](curator.md) is an adopter-project role and does not boot in the template repo ([FWADR-026](adrs/fwadr-026-curator-role.md) Revision v1.1), so framework-corpus curation lands here. Run `bash docs/dev_framework/_stubs/scripts/doc_churn_audit.sh` from the repo root — in the template the script is an un-synced stub, so there is no `scripts/` copy. Same signals, same doctrine: it ranks, it does not recommend, and dispositions are the user's per item.
- **Lands `fwreq-*` framework-update requests filed by the Curator.** The second inbound path into this role ([FWADR-026](adrs/fwadr-026-curator-role.md)). The [Curator](curator.md) is mechanically fenced out of `docs/dev_framework/*` and `.claude/hooks/*`, so its framework findings arrive as `docs/framework_exceptions/fwreq-NNN-<slug>.md` files that the **user** carries here — not via Strategist triage, because a framework change is not the Strategist's to approve. Each request states an observed friction and a desired outcome, never a design; the how is this role's call. Dispose by updating the request's `Status:` (`landed` / `declined`) so the adopter-side Curator stops re-surfacing it.
